...
replaceHoles holeIdentSupply p =
    evalState (repHoles p) $
        HoleCandidates 0 $
            GHC.listSplitUniqSupply holeIdentSupply
  where
    repHoles :: GHC.CoreProgram -> State HoleCandidates GHC.CoreProgram
...
replaceCaseBinds = transformBi repBinds
  where
    repBinds :: GHC.CoreExpr -> GHC.CoreExpr
    ...
    repBinds (GHC.Case scrutinee wild t as) | isWild wild = GHC.Case scrutinee wild t (map (sub scrutinee wild) as)
      where
        sub :: GHC.CoreExpr -> GHC.Var -> GHC.Alt GHC.Var -> GHC.Alt GHC.Var
...
alpha
    :: String -> GHC.CoreProgram -> (GHC.CoreProgram, Map.Map GHC.Var GHC.Var)

-- | Do renaming and return map of renamed variables
alpha fname cs = (prog, env state)
  where
    (prog, state) = runState st (initSt{exerName = fname})
    st = mapM alphaR cs
...
resetState st =
    st
        { topBindCount = topBindCount st + 1
        , freshCaseBindVar = 0
        , freshVar = 0
        , freshBindVar = 0
        }
    ...
    | otherwise = do
        ...
        if GHCOcc.getOccString v == name -- ! this differs from Matilda code, check if something went wrong
            then return v
                ...
                Just n
                    | GHC.varType n `GHC.eqType` GHC.varType v -> return n
...
renameV vty v = do
    ...
    let v' =
            GHC.setVarName
                v
                (makeName vname (GHC.getUnique v) (GHCOcc.getSrcSpan (GHC.varName v)))
...
aRename l@(GHC.Lam b e) = do
    --  trace ("Lambda var:" ++ show b ++ " isTyVar: " ++ show (isTyVar b)) $
