    ...
    where inlineBind :: GHC.CoreProgram -> GHC.CoreProgram -> GHC.CoreProgram
          ...
          inlineBind acc (bind:binds)
              |   not (isSpecVar v) -- remove bindings generated by compiler, like $trModule
                ...
                , length usedBy == 1 -- if the top level binding is only used once, inline it
                , notTyCon $ head usedBy = -- ? confused with this example
                            let newBinds  = inline bind usedBy
                                remaining = fmap GHC.deBruijnize (acc ++ binds) \\ fmap GHC.deBruijnize usedBy
                                -- ! Next time start at here, check if we can convert from deBruijn to normal, or use an auxiliary list to track
                            in inlineBind newBinds $ remaining 
              | otherwise = inlineBind (bind:acc) binds
                where v :: GHC.Var
                      ...
                      usedBy = getBindersByVar (acc ++ binds) bind
