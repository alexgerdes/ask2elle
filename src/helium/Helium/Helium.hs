...
compileFn
    ...
    -> Type
    -> CodeSnippet
    -> IO (Either (Helium.HeliumError, T.Text) Helium.Module)
compileFn fnName fnType fnImplementation = do
    -- isDefined <- runExceptT $ Helium.typeOf fnName fnImplementation'
    compilationResult <-
        Helium.compile
            False
            (typeDecl `T.append` fnImplementation')
            Helium.askelleDefaultOptions
...
compileCode
    :: Module
    -> CodeSnippet
    -> Helium.AskelleOptions
    -> IO (Either (Helium.HeliumError, T.Text) Helium.Module)
compileCode moduleName code options = do
    compilationResult <-
        Helium.compile False (escape code) options{Helium.moduleName = Just moduleName}
    pure $ fmap Helium.getModule compilationResult
