...
import GHC.Types.Var (Var (..), isId, isTcTyVar, isTyVar, tyVarKind)
import GHC.Unit.Types
import GHC.Utils.Outputable (Outputable (ppr), showSDocUnsafe)
import GhcLib.GHCRelated.ShowCore

import Data.Generics.Uniplate.Data (Biplate, rewriteBi)
...
instance Similar (Bind Var) where
    ...
    (Rec es) ~> (Rec es') = es ~> es'
    (NonRec v e) ~> (NonRec v' e')
        | isTrModuleVar v, isTrModuleVar v' = True -- disregard module info binders
        | otherwise = v ~> v' && e ~> e'
...
instance Similar [Alt Var] where
    ...
    (x : xs) ~> ys = matchUnordered x ys && xs ~> ys
      where
        matchUnordered x [] = False
        matchUnordered x (y : ys)
            | x ~> y = True
...
instance Similar DataCon where
    x ~> y = dcName x == dcName y
      where
        dcName = getOccString . dataConName
    (~=) = (~>)
...
r = rewriteBi remTick
  where
    remTick :: CoreExpr -> Maybe CoreExpr
    remTick ex = case ex of
        (Tick _ e) -> Just e
        e -> Nothing

class
    (Biplate b CoreProgram, Biplate b CoreBind, Biplate b CoreExpr) =>
    BiplateFor b
instance BiplateFor CoreProgram
instance BiplateFor CoreBind
instance BiplateFor CoreExpr
