...
ppDeclaration d =
    T.pack . show $
        PP.text_Syn_Declaration
            (PP.wrap_Declaration (PP.sem_Declaration d) PP.Inh_Declaration)

ppExpression :: Helium.Expression -> T.Text
ppExpression e =
    T.pack . show $
        PP.text_Syn_Expression
            (PP.wrap_Expression (PP.sem_Expression e) PP.Inh_Expression)
