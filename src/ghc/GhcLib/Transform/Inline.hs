...
inlineBinds exfun = inlineBind []
  where
    inlineBind :: GHC.CoreProgram -> GHC.CoreProgram -> GHC.CoreProgram
    ...
    inlineBind acc (bind : binds)
        ...
        , length (fst usedBy) == 1
        , -- \* right now if the top level binding is only used once, inline it
...
insertBind (GHC.Rec ls@((v, e) : es)) bs = map insertR bs
  where
    insertR :: GHC.CoreBind -> GHC.CoreBind
    insertR bind@(GHC.NonRec b (GHC.Lam x ex)) =
        GHC.NonRec b $
            GHC.Lam x $
                GHC.Let (GHC.Rec ((uv, e') : es)) (subsVar uv v ex)
    insertR bind@(GHC.NonRec b ex) =
        GHC.NonRec b $
            GHC.Let (GHC.Rec ((uv, e') : es)) (subsVar uv v ex)
...
recToLetRec letRecSupply p =
    evalState (recToLR p) $ HoleCandidates 0 $ GHC.listSplitUniqSupply letRecSupply
  where
    recToLR :: GHC.CoreProgram -> State HoleCandidates GHC.CoreProgram
    recToLR [] = pure []
        ...
        (GHC.Rec ((v, e) : ls)) -> do
            ...
            let topLevelBinder = fresh holeCandidate v
                ...
                b' =
                    GHC.NonRec
                        v
                        (GHC.Let (GHC.Rec ((topLevelBinder, e') : ls)) (GHC.Var topLevelBinder))
            -- bs' <- recToLR bs
